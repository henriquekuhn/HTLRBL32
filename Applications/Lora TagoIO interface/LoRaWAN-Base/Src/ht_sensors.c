...
#define APP_SENSORS_DUTYCYCLE                            15000
...
uint8_t dados [2];
uint16_t devAdd_read = 0xBF;
uint16_t devAdd_write = 0xBE;
...
void calibrateHumidity(void){
	...
	H0_rH = H0_rH_x2 >> 1; //H0_rH_x2/2;
	H1_rH = H1_rH_x2 >> 1; //H1_rH_x2/2;
	...
	H0_T0_OUT = ((dados[1] & 0xFF) * 256) + (dados[0] & 0xFF);;//(dados[1] << 8) | dados[0];
	...
	H1_T0_OUT = ((dados[1] & 0xFF) * 256) + (dados[0] & 0xFF);///(dados[1] << 8) | dados[0];
...
void initSensors(void){
			...
			TimerInit( &sensorsTimer, OnSensorsTimerEvent );
			//Sets alarm total time
      TimerSetValue( &sensorsTimer, APP_SENSORS_DUTYCYCLE);		
			//Callback call
...
float readPressure(void){
	...
	P = P * 0.0009869233; // return pressure in atm
